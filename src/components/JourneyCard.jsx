 ...
 */
function LineIconsDisplay({ item, isLineDisrupted, disruptedColor }) {
  const { lineKeys, line } = item;
  // `line` peut correspondre au filtre de recherche (ex. "B,C5") ; pour
  // une seule ligne, l'icône doit toujours représenter la ligne du trajet.
  const displayedLine = lineKeys?.[0] || line;
        ...
        {displayKeys.map((lk, i) => (
          <div key={`${lk}-${i}`} className={`absolute ${positions[i]}`}>
            <LineWithBadge
              ...
              size="w-6 h-6"
              badgeSize="size-3"
              isDisrupted={isLineDisrupted(lk)}
              disruptedColor={disruptedColor}
            />
          </div>
        ))}
        {lineKeys.length > 4 && (
      ...
      <>
        {lineKeys.map((lk, i) => (
          <div
            ...
            style={{
              transform:
                i === 0 ? "translate(-10%, -10%)" : "translate(10%, 10%)",
            }}
          >
            <LineWithBadge
              lineKey={lk}
  ...
  return (
    <div className="w-14 h-14 flex items-center justify-center relative">
      <LineIcon lineKey={displayedLine} size="w-12 h-12" />
      {isLineDisrupted(displayedLine) && (
        <span
